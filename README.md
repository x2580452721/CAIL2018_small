...
> **提示**：如果你遇到 HuggingFace 下载慢：可以设置镜像/代理，或保持当前方式即可。
...
```
你已得到：`first_stage/train_split.json` 和 `first_stage/valid_split.json`

### 4.2 构建 label_map.json
```bash
python src/data/build_label_map.py
```
**输出**：`first_stage/label_map.json`  
...
将 `model_name` 改为 `"thunlp/Lawformer"`，输出目录建议改为 `outputs/exp_lawformer/`。
...
最终测试口径使用 `test.py`（写 preds + metrics），`threshold=0.2`。
...
**RoBERTa 最终 test 指标（threshold=0.2）**：
...
### 7.2 网页端
网页通过 `fetch("/predict")` 调用接口，输入文本后返回罪名/法条/刑期。
...
- `encoder/`：fine-tuned 后的编码器权重
- `tokenizer/`：分词器文件
- `multitask_head.pt`：三任务分类头参数
- `best_metrics.json`：验证集最优指标记录

---

## 9. 数据集来源 (Reference)

本项目使用的数据集来源于 **CAIL2018 (Chinese AI and Law 2018)**。
